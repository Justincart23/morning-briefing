...
def fetch_newsapi_sports():
    """
    Fetch sports headlines from NewsAPI.org.
    Free tier: 100 req/day. Requires NEWS_API_KEY env var.
    Sign up free at https://newsapi.org
    """
    if not NEWS_API_KEY or not REQUESTS_OK:
        return []
    try:
        # Search for top sports stories across NFL, NBA, MLB, NHL
        queries = [
            "NFL OR NBA OR MLB OR NHL OR Eagles OR Phillies OR 76ers OR Flyers",
        ]
...
def sports_brief():
    """
    Sports section — pulls from NewsAPI (works from GitHub Actions).
    Falls back to static context if no API key set.
    """
    month = NOW.month

    # Philly teams for badge highlighting
    philly_keywords = ["phillies", "eagles", "76ers", "sixers", "flyers",
                       "philadelphia", "bryce harper", "jalen hurts", "tyrese maxey"]

    # League keywords for categorization
    league_kw = {
        "⚾ MLB":  ["mlb", "baseball", "phillies", "yankees", "dodgers", "braves",
                    "mets", "cubs", "red sox", "astros", "world series", "bryce harper",
                    "home run", "strikeout", "pitcher", "inning"],
        "🏀 NBA":  ["nba", "basketball", "76ers", "sixers", "celtics", "lakers",
                    "warriors", "heat", "knicks", "nba finals", "playoffs",
                    "tyrese maxey", "lebron", "curry", "jokic", "tatum"],
        "🏈 NFL":  ["nfl", "football", "eagles", "cowboys", "chiefs", "patriots",
                    "49ers", "ravens", "draft", "nfl draft", "jalen hurts",
                    "quarterback", "touchdown", "super bowl", "training camp"],
        "🏒 NHL":  ["nhl", "hockey", "flyers", "rangers", "bruins", "penguins",
                    "stanley cup", "playoffs", "goal", "goalie", "power play"],
    }

    # Fetch from NewsAPI
    all_articles = fetch_newsapi_sports()

    sport_html = ""
    used = set()

    if all_articles:
        # Bucket articles by sport
        buckets = {league: [] for league in league_kw}
        general = []

        for art in all_articles:
            text = (art["title"] + " " + art["desc"]).lower()
            placed = False
            for league, kws in league_kw.items():
                if any(kw in text for kw in kws):
                    if art["title"] not in used:
                        buckets[league].append(art)
                        used.add(art["title"])
                        placed = True
                        break
            if not placed and art["title"] not in used:
                general.append(art)
                used.add(art["title"])

        # Render each league bucket (up to 3 stories each)
        for league, articles in buckets.items():
            show = articles[:3]
            if not show:
                continue
            sport_html += f'<div style="font-size:10px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.07em;margin:14px 0 6px;padding-bottom:4px;border-bottom:1px solid #f1f5f9">{league}</div>'
            for art in show:
                title = art["title"]
                desc  = art["desc"]
                link  = art["link"]
                src   = art["source"]
                tl    = title.lower()
                is_philly = any(kw in tl or kw in desc.lower() for kw in philly_keywords)
                philly_badge = ' <span style="font-size:9px;font-weight:800;background:#003f8f;color:#fff;padding:1px 5px;border-radius:3px;vertical-align:middle">PHILLY</span>' if is_philly else ""
                src_badge = f'<span style="font-size:10px;color:#94a3b8;margin-right:4px">{src}</span>' if src else ""
                desc_html = f'<div style="font-size:11px;color:#64748b;margin-top:2px;line-height:1.5">{desc}</div>' if desc else ""
                sport_html += f'<div style="padding:7px 0;border-bottom:1px solid #f8fafc">{src_badge}<a href="{link}" style="font-size:13px;font-weight:700;color:#0f172a;text-decoration:none">{title}</a>{philly_badge}{desc_html}</div>'

        # Any leftover top stories not bucketed
        if general:
            sport_html += '<div style="font-size:10px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.07em;margin:14px 0 6px;padding-bottom:4px;border-bottom:1px solid #f1f5f9">🏆 More Sports</div>'
            for art in general[:3]:
                src_badge = f'<span style="font-size:10px;color:#94a3b8;margin-right:4px">{art["source"]}</span>' if art["source"] else ""
                sport_html += f'<div style="padding:7px 0;border-bottom:1px solid #f8fafc">{src_badge}<a href="{art["link"]}" style="font-size:13px;font-weight:700;color:#0f172a;text-decoration:none">{art["title"]}</a></div>'

    else:
        # No API key or request failed — show setup prompt
        sport_html = """
        <div style="padding:14px;background:#fef9c3;border-left:3px solid #eab308;border-radius:4px;margin-bottom:12px">
          <div style="font-size:12px;font-weight:800;color:#854d0e;margin-bottom:4px">⚙️ One-time setup needed for full sports coverage</div>
          <div style="font-size:12px;color:#713f12;line-height:1.6">
            1. Go to <strong>newsapi.org</strong> → sign up free (30 sec)<br>
            2. Copy your API key<br>
            3. Go to GitHub → your repo → Settings → Secrets → add <strong>NEWS_API_KEY</strong><br>
            Full NBA, NFL, MLB, NHL coverage will appear automatically next run.
          </div>
        </div>"""

    # Philly context block
    philly_block = """
    <div style="margin-top:14px;padding:10px 12px;background:#f0f4ff;border-left:3px solid #003f8f;border-radius:4px">
      <div style="font-size:10px;font-weight:800;color:#1e3a8a;text-transform:uppercase;margin-bottom:5px">🦅 Your Philadelphia Teams</div>
      <div style="font-size:12px;color:#1e293b;line-height:1.8">
        ...
        <span style="font-size:11px;color:#64748b">Philly stories tagged above in blue. Instant common ground with any PA client, banker, or manager.</span>
      </div>
    </div>"""

    hooks = {
        "Monday":    "Did you catch any games this weekend? What'd you think?",
        "Tuesday":   "Big game last night — did you see how it ended?",
        "Wednesday": "Midweek check — how are your teams looking?",
        "Thursday":  "Almost the weekend. Any games on your radar?",
        "Friday":    "Big sports weekend coming up — got plans around any of the games?",
        "Saturday":  "So much going on today — anything you're locked in on?",
    ...
    <div style="margin-top:12px;padding:12px 14px;background:#f0fdf4;border-left:3px solid #22c55e;border-radius:4px">
      ...
      <div style="font-size:11px;color:#64748b;margin-top:4px">Sports = instant rapport. Know one story per sport and you own any room.</div>
    </div>"""

    src_note = '<div style="margin-top:8px;font-size:10px;color:#94a3b8">Source: NewsAPI.org · US Sports headlines</div>' if all_articles else ""
    content = sport_html + philly_block + footer + src_note
