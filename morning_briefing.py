...
def sports_brief():
    ...
    for label, url in sport_feeds:
        ...
        if items:
            sport_html += f'<div style="font-size:10px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.07em;margin:14px 0 6px;padding-bottom:4px;border-bottom:1px solid #f1f5f9">{label} <span style="color:#cbd5e1;font-weight:400;font-size:9px">via ESPN</span></div>'
            for item in items:
                desc = item.get("desc", "")
                desc_html = f'<div style="font-size:11px;color:#64748b;margin-top:2px;line-height:1.4">{desc[:120]}{"…" if len(desc) > 120 else ""}</div>' if desc else ""
                sport_html += f"""<div style="padding:5px 0">
                  <a href="{item['link']}" style="font-size:13px;font-weight:600;color:#0f172a;text-decoration:none">{item['title']}</a>
                  {desc_html}
                </div>"""
    ...
    hooks = {
        "Monday":    "\"Did you catch anything this weekend? The [NBA Finals / game] was wild.\"",
        "Tuesday":   "\"Game 1 last night — did you see [result]? That fourth quarter was something.\"",
        "Wednesday": "\"Midweek already — the [series / standings] are really heating up.\"",
        "Thursday":  "\"Almost Friday — and there's a game tonight. You watching?\"",
        "Friday":    "\"Big sports weekend coming up — got any plans around the games?\"",
    ...
    footer = f"""
    <div style="margin-top:16px;padding:12px 14px;background:#f0fdf4;border-left:3px solid #22c55e;border-radius:4px">
      ...
      <div style="font-size:11px;color:#4ade80;margin-top:4px">Use this with clients, bankers, and managers during coffee chats. Sports = instant rapport.</div>
    </div>"""

    content = sport_html + footer + '<div style="margin-top:8px;font-size:10px;color:#94a3b8">Source: ESPN RSS</div>'
